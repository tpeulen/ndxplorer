...
def _row(panel, key):
    return next(r for r in panel.parameter_rows() if r["key"] == key)
...
def test_a_vector_is_an_expandable_row(app):
    ...
    children = [r for r in panel.parameter_rows() if r["parent"] == "gG/gR[]"]
    assert [r["name"] for r in children] == ["(global)", "HF", "LF"]
    assert "± 0.02" in children[2]["note"]
    assert not panel.cell_editable(parent, "value") and panel.cell_editable(parent, "fixed")
    panel.edit_parameter(parent, "fixed", False)
    assert not panel.group.parameters_all_dict["gG/gR[LF]"].fixed
    panel.edit_parameter(children[1], "value", 0.7)
    assert panel.values()["gG/gR[HF]"] == 0.7
    panel.expanded.add("gG/gR[]")
    draw(app)
    binding = panel.form.tables["parameter_rows"]
...
def test_make_vector_from_the_menu_and_back(app):
    f = feature(app)
    panel = f.constants
    f.constants.parameter_menu(_row(panel, "Bg"), "value", (100.0, 200.0))
    popup, _ = app.popup
    ...
    dialog.ok()
    assert [r["name"] for r in panel.parameter_rows() if r["parent"] == "Bg[]"] == \
        ["(global)", "0", "1", "2"]
    f.clipboard = "1\t2\t3"
    panel.parameter_menu(_row(panel, "Bg[]"), "value", (100.0, 200.0))
    popup, _ = app.popup
    ...
    assert "Bg[]" not in [r["key"] for r in panel.parameter_rows()]
...
def test_lo_and_hi_show_infinity_until_a_bound_is_typed(app):
    ...
    bg = _row(panel, "Bg")
    assert (bg["lo"], bg["hi"]) == ("−∞", "∞")
    panel.edit_parameter(bg, "lo", "0.5")
    bg = _row(panel, "Bg")
    assert (bg["lo"], bg["hi"]) == (0.5, "∞")
    panel.edit_parameter(bg, "lo", "")
